...
applicationId=$(curl -s -X GET --header 'Accept: application/json' --header 'Range: items=0-1000' --header "Asc_xsrf_token: $sessionId" --header "Cookie: asc_session_id=$sessionId;" 'https://winappscan.lab.local:9443/ase/api/applications' --insecure | jq | jq '.[] | select(.name == "'"$applicationName"'")' | grep -oP '(?<="id": ")[^"]*')
